...
public class ForgottenPlanetEnchantedGrassPatch extends WorldGenerator
{
    private int blockIndex;

    public ForgottenPlanetEnchantedGrassPatch(int par1)
    {
        ...
        else
        {
            ...
            for (i1 = 0; i1 < 16; ++i1)
            {
                for (j2 = 0; j2 < 16; ++j2)
                {
                    for (i2 = 0; i2 < 8; ++i2)
                    {
                        ...
                        {
                            par1World.setBlock(par3 + i1, par4 + i2 + 1, par5 + j2, i2 >= 4 ? 0 : this.blockIndex, 0, 2);
                        }
                    }
                }
            }
            ...
            for (i1 = 0; i1 < 16; ++i1)
            {
                for (j2 = 0; j2 < 16; ++j2)
                {
                    for (i2 = 4; i2 < 8; ++i2)
                    {
                        if (aboolean[(i1 * 16 + j2) * 8 + i2] && par1World.getBlockId(par3 + i1, par4 + i2 - 1, par5 + j2) == Blocks.forgottenDirt.blockID && par1World.getSavedLightValue(EnumSkyBlock.Sky, par3 + i1, par4 + i2, par5 + j2) > 0)
                        {
                            BiomeGenBase biomegenbase = par1World.getBiomeGenForCoords(par3 + i1, par5 + j2);

                            if (biomegenbase.topBlock == Block.mycelium.blockID)
                            {
                                par1World.setBlock(par3 + i1, par4 + i2, par5 + j2, Blocks.enchantedGrass.blockID, 0, 2);
                            }
                            else
                            {
                                par1World.setBlock(par3 + i1, par4 + i2, par5 + j2, Blocks.enchantedGrass.blockID, 0, 2);
            ...
            if (Block.blocksList[this.blockIndex].blockMaterial == Material.water)
            {
                for (i1 = 0; i1 < 16; ++i1)
                {
                    for (j2 = 0; j2 < 16; ++j2)
                    {
                        ...
                        {
                            par1World.setBlock(par3 + i1, par4 + b0, par5 + j2, Blocks.enchantedGrass.blockID, 0, 2);
                        }
                    }
                }
            }
